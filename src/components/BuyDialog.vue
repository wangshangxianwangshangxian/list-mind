<template>
  <Dialog :title="proxy.$lang('付费阅读')" bg="bg-black-30" @c_close="emits('c_close')">
    <div class="w-96">
      <div class="flex justify-center items-center h-10 text-2xl gap-2 mt-4">
        支付 {{ props.amount }} ETH
      </div>
      <div class="mt-4">
        <button @click="onbuy" class="p-2 border rounded w-full cursor-pointer flex justify-center gap-2 items-center hover:border-orange-500">
...
import { ACCOUNT2, MESSAGE_TYPE, PAY } from '@/stores/constant';
...
const onbuy = async () => {
  ...
  let resp = await get('pre-buy-mind', { address: props.address })
  if (resp.code !== ERRORCODE.SUCCESS) {
    return proxy.$message(proxy.$lang('支付异常'))
  }

  const str = `${props.address} ${resp.data.recipit}`
  resp = await pay(PAY.ETHER, ACCOUNT2, props.amount, Web3.utils.stringToHex(str))
  if (resp.code === ERRORCODE.SUCCESS) {
    const param = {
      tx_hash : resp.data.tx_hash
    }
    const result = await post('buy-mind', param)
    if (result.code === ERRORCODE.SUCCESS) {
      proxy.$message(proxy.$lang('付费成功!'))
      emits('c_close')
    }
    ...
    proxy.$message(resp.message, MESSAGE_TYPE.WARN)
  }
}
</script>