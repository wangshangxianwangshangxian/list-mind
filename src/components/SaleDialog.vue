<template>
  <Dialog :title="proxy.$lang('设置付费阅读')" bg="bg-black-30" @c_close="emits('c_close')">
    <div class="w-96">
      <div class="mt-4">
        <div class="flex justify-center items-center h-10 text-2xl gap-2">
          支付
          <input type="number" v-model="amount" class="focus:outline-none text-center remove-arrows inline w-40 focus:border-gray-200 border-b border-white" :placeholder="proxy.$lang('输入金额')" />
          ETH
        </div>
        <p class="text-center my-2">到</p>
        <div class="flex gap-2 items-center">
          <input ref="ref_recipit" type="text" v-model="recipit" :placeholder="proxy.$lang('输入收款钱包地址')" class="w-full h-10 focus:outline-none text-sm text-center" />
        </div>
      </div>
      <div class="mt-4">
        <button @click="onset" class="p-2 border rounded w-full cursor-pointer flex justify-center gap-2 items-center hover:border-orange-500">
          <svg xmlns="http://www.w3.org/2000/svg" class="size-5" fill="none" viewBox="0 0 50 48" id="metamask">
            <path fill="#E2761B" stroke="#E2761B" stroke-linecap="round" stroke-linejoin="round" d="M46.6094 2L27.88 15.9106L31.3435 7.70353L46.6094 2Z"></path><path fill="#E4761B" stroke="#E4761B" stroke-linecap="round" stroke-linejoin="round" d="M3.37177 2L21.9506 16.0424 18.6565 7.70353 3.37177 2zM39.8706 34.2447L34.8824 41.887 45.5553 44.8235 48.6235 34.4141 39.8706 34.2447zM1.39529 34.4141L4.44471 44.8235 15.1176 41.887 10.1294 34.2447 1.39529 34.4141z"></path>
      ...
      <div class="flex mt-4 flex-col gap-1 text-gray-800 text-sm">
        <p>1. 设置金额后，读者需要支付预定金额才能永久阅读</p>
        <p>2. 可随时修改金额</p>
        <p>3. 当天收益于1个工作日内转入指定账号</p>
      </div>
    </div>
  </Dialog>
</template>
...
const ref_recipit =  ref(null)
onMounted(()      => ref_recipit.value.focus())
const amount      =  ref(props.amount)
const recipit     =  ref(props.recipit)

// 设置
const onset = async () => {
  if (!props.address) {
    return proxy.$message(proxy.$lang('address 不存在'))
  }
  const data = { address: props.address, recipit: recipit.value, value: amount.value }
  const resp = await post('set-sale-info', data)
