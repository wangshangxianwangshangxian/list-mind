<template>
  <Dialog title="保存到远端" bg="bg-black-30" @c_close="onclose">
    <div class="w-96 flex flex-col" v-loading="loading">
      <div class="flex justify-center items-center h-20 text-2xl">
        支付 {{ REMOTE_AMOUNT }} ETH
      </div>
      <div>
        <button v-loading="loading" @click="onpay" class="p-2 border rounded w-full cursor-pointer flex justify-center gap-2 items-center hover:border-orange-500">
...
import { ACCOUNT, MESSAGE_TYPE, PAY, REMOTE_AMOUNT } from '@/stores/constant';
...
const onpay = async () => {
  loading.value = true
  const resp = await pay(PAY.ETHER, ACCOUNT, REMOTE_AMOUNT, props.address)
...
const emits = defineEmits(['c_close', 'c_payed'])
